function [ fh ] = plot_rasterSpeechTaskBursts( figdir, figtitle, Stim, trange, BB, BurstColor, order, markers )
...
for i = 1:size(Stim.DD,1)
    if ~isempty(BB(i).ii)
        indD = find(Stim.DD(i,:));
        rectangle('Parent',rasterh,'Position', ...
            [plott(indD(BB(i).ii(1))), ...
            i, ...
            plott(indD(BB(i).ii(end)+1))-plott(indD(BB(i).ii(1))), ...
            0.9], ...
            'FaceColor',BurstColor{i},'EdgeColor',BurstColor{i});
    end
    
    if ~isempty(markers)
        line('Parent',rasterh,'XData',marker(i)*[1 1],...
              'YData',[i ],'Tag','single_spike', 'LineWidth', 2);
    end
end
