coding_dir = '/Users/Witek/Dropbox/Richardson Lab/Speech Project/Decoding/coding files';
...
for ii = 1:length(rec_idx)
    ...
    %load([subjectName,'_Session',num2str(ii)]);
    
    AudioStart = SpeechTrials( ...
        find(strcmp([subjectName,'_Session',num2str(ii),'.mat'], ...
        SpeechTrials(:,1))),12);
    ...
    for trial=1:nTrials
        ...
        if trial<=length(AudioStart) && ~isempty(AudioStart{trial})
            ...
            ResponseTimes(trial) = EventTimes1(StimulusEvent1) + AudioStart{trial}/sampRate;
        else 
    ...
    for jj=1:length(electrodeList)
        ...
        if ~isempty(spkFile) && ~isempty(unitList.(electrodeList{jj}){ii}.units)
            ...
            for n=1:nUnits
                ...
                if isempty(trialRange); trialRange = [1 60]; end
...
%                 end
                ...
                k_ind=0;
                for kk = 1:length(spike_trials)
                %for kk = 1:unit.nTrials
                    tnum = unit.trial(spike_trials(kk)).stim;
                    if ~isempty(AudioStart{tnum})
                        audioInd = unit.trial(spike_trials(kk)).EventInds(4) + round(AudioStart{tnum}/sampRate*unit.spkSampRate);
                        offset = unit.trial(spike_trials(kk)).EventInds(1);
                        trial_inds = audioInd+respWind-offset;
                        if(trial_inds(1) < 1)
                            fprintf('%f time between trial start and cue\n', (unit.trial(spike_trials(kk)).EventInds(4)-unit.trial(spike_trials(kk)).EventInds(1))/unit.spkSampRate);
                            fprintf('%f Audio latency\n', AudioStart{tnum}/sampRate);
