...
for idx=1:length(DATA)
    ...
    CuePause.T = cellfun( @(x,y) time(indDD{y}(min(x))), ...
        {DATA(idx).SpeechOnset.PP(:).ii}, num2cell(1:length(indDD)), ...
        'uniformoutput', false)';
    % convert timing data for trials with no burst detected to NaN
    idxNoPause = find(cellfun(@isempty, CuePause.T));  
    CuePause.T(idxNoPause) = {NaN};
    CuePause.T = cellfun(@double, CuePause.T);
    ...
    if length(RT) > 3
        ...
        saveas(fh,[figdir,'/TimingCorrelationsCue2Pause/',figtitle,'.pdf'], 'pdf');
        close(fh);
    else
        CuePause.rho=0;
        CuePause.pval=1;
