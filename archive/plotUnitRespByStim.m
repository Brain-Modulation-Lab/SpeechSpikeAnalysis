...
[fh, rasterh, meanh, ~, labelh] = makeRasterIFRplot(sprintf('%s,Word List %d,%s electrode,Unit %d, %s', subjectName, wl, electrodeList{trode}, n, contrastName));
...
medianCueTime = NaN*zeros(length(groups),1);
...
for ll = 1:length(groups) %loop through groups, plot them separately
    ...
    for mm=1:nGroupTrials(ll)
        ...
        cueTime(kk) = (unit.trial(t).EventInds(4)-audioInd)/unit.spkSampRate;
    ...
    if nGroupTrials(ll) ~= 0
        ...
        medianCueTime(ll) = nanmedian(cueTime);
        plot(meanh, [medianCueTime(ll), medianCueTime(ll)], meanh.YLim, '--', 'Color', colorlist{ll});
    end
end
% Annotate the plot
