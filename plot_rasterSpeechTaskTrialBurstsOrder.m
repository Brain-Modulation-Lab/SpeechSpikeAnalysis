function [ fh ] = plot_rasterSpeechTaskTrialBurstsOrder( figdir, figtitle, Stim, Trial, trange, BurstColor, order, markers, MarkerColor )
...
for i = order'%1:size(Stim.DD,1)
    k=k+1;
    if ~isempty(Trial.BB(i).ii)
        indD = find(Trial.DD{i});
        nSpikesPreCue = nnz(Trial.DD{i}(1:Trial.idxEventTimes{i}(2))); % WJL added 3/8/2018 to account for burst indexing starting from Cue
        rectangle('Parent',rasterh,'Position', ...
            [Trial.timeCue{i}(indD(nSpikesPreCue + Trial.BB(i).ii(1))), ...
            k, ...
            Trial.timeCue{i}(indD(nSpikesPreCue + Trial.BB(i).ii(end)-1))-Trial.timeCue{i}(indD(nSpikesPreCue + Trial.BB(i).ii(1))), ...
            0.9], ...
